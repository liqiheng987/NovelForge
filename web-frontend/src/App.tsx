...
export default function App() {
  ...
  const removeChapter = async (chapterId: string) => { const result = await api<{ affected_nodes?: ImpactHighlight[] }>(`/chapter/delete?chapter_id=${encodeURIComponent(chapterId)}`, { method: "DELETE" }); if (result.affected_nodes) setImpactHighlights(result.affected_nodes); if (currentProjectId) await loadChapters(currentProjectId); };
