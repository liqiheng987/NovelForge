...
import type { BranchComparison, ChatMessage, ChatSession, Chapter, ChapterDraft, ChapterVersion, CreationAction, Fact, FileInfo, GenerationTask, ImpactHighlight, Mode, NovelMaterial, Paper, Project, ProjectSettings as ProjectSettingsConfig, StoryNode, Toast, UniverseRule } from "./types";
...
export default function App() {
  ...
  const [deletedChapters, setDeletedChapters] = useState<ChapterVersion[]>([]);
  const [recoverableTask, setRecoverableTask] = useState<GenerationTask | null>(null);
  ...
  const loadChapters = useCallback(async (projectId: string | null) => {
    if (!projectId) { useChapterStore.getState().setChapters([]); setDeletedChapters([]); return; }
    const [chapterItems, deletedItems] = await Promise.all([
      api<Chapter[]>(`/chapters?project_id=${encodeURIComponent(projectId)}`),
      api<ChapterVersion[]>(`/chapters/trash?project_id=${encodeURIComponent(projectId)}`),
    ]);
    useChapterStore.getState().setChapters(chapterItems); setDeletedChapters(deletedItems);
  }, []);
  ...
  const refreshGenerationTasks = useCallback(async (sessionId: string) => { const tasks = await api<GenerationTask[]>(`/generation/tasks?session_id=${encodeURIComponent(sessionId)}`); setRecoverableTask(tasks[0] ?? null); return tasks; }, []);
  ...
  const switchSession = useCallback(async (sessionId: string) => {
    ...
    await refreshSessions(result.project_id); await loadProjectSettings(result.project_id); await loadChapters(result.project_id); await refreshGenerationTasks(sessionId); useModeStore.getState().setLocalMode(result.session.mode || "guided");
  }, [loadChapters, loadProjectSettings, refreshGenerationTasks, refreshSessions]);
  ...
  const runChat = async (question: string, paperSourceMessageId: string | null, regenerateAssistantId?: string, selectedSnapshot?: string[], creationAction: CreationAction = "auto", chapterTargetWords?: number, resumeTask?: GenerationTask) => {
    if (!activeSessionId || generating) return false;
    if (recoverableTask && !resumeTask) { notify("请先继续或放弃未完成的生成任务", "info"); return false; }
    if (!apiReady) { setShowSettings(true); return false; }
    const requestId = resumeTask?.id ?? crypto.randomUUID(); const snapshot = resumeTask?.request_payload.selected_material_ids ?? selectedSnapshot ?? [...selectedIds]; const state = useChatStore.getState(); const assistantPending = `pending-${crypto.randomUUID()}`; const userPending = `pending-user-${crypto.randomUUID()}`; const timestamp = new Date().toISOString();
    ...
    else if (resumeTask) state.setMessages([...state.messages, { id: assistantPending, session_id: activeSessionId, role: "assistant", content: "", selected_material_ids: [], has_paper: false, paper: null, created_at: timestamp }]);
    ...
    try {
      const response = await agentFetch("/chat", { method: "POST", body: JSON.stringify({ request_id: requestId, session_id: activeSessionId, project_id: currentProjectId, mode: resumeTask ? resumeTask.request_payload.mode : mode, message: question, selected_material_ids: snapshot, api_config: apiConfig(), regenerate_assistant_id: regenerateAssistantId ?? null, paper_source_message_id: paperSourceMessageId, creation_action: creationAction, chapter_target_words: chapterTargetWords ?? null }), signal: controller.signal });
      ...
      } else { await refreshSessions(currentProjectId ?? ""); await refreshGenerationTasks(activeSessionId); }
      return true;
    } catch (error) { const aborted = error instanceof DOMException && error.name === "AbortError"; notify(aborted ? "已停止生成，任务进度已保留，可稍后继续" : error instanceof Error ? error.message : "对话生成失败", aborted ? "info" : "error"); try { await new Promise((resolve) => window.setTimeout(resolve, 120)); await switchSession(activeSessionId); } catch { state.setMessages(state.messages.filter((item) => item.id !== assistantId && item.id !== assistantPending)); } if (!regenerateAssistantId && !resumeTask) useMaterialSelectionStore.getState().setSelectedIds(snapshot); return false; } finally { abortRef.current = null; setGenerating(false); setGenerationStage(null); }
  };

  const cancelGeneration = () => abortRef.current?.abort();
  const resumeGenerationTask = async () => { if (!recoverableTask || recoverableTask.session_id !== activeSessionId) return; const request = recoverableTask.request_payload; await runChat(request.message, request.paper_source_message_id, request.regenerate_assistant_id ?? undefined, request.selected_material_ids, request.creation_action, request.chapter_target_words ?? undefined, recoverableTask); };
  const dismissGenerationTask = async () => { if (!recoverableTask) return; try { await api(`/generation/tasks/${encodeURIComponent(recoverableTask.id)}`, { method: "DELETE" }); if (activeSessionId) await refreshGenerationTasks(activeSessionId); notify("未完成的生成任务已放弃", "info"); } catch (error) { notify(error instanceof Error ? error.message : "放弃生成任务失败"); } };
  ...
  const renameProject = async (title: string) => { if (!currentProjectId) return; try { const updated = await api<Project>(`/projects/${encodeURIComponent(currentProjectId)}`, { method: "PUT", body: JSON.stringify({ title }) }); setProjects((current) => current.map((project) => project.id === updated.id ? { ...project, ...updated, active: project.active } : project)); notify("作品名称已更新", "success"); } catch (error) { notify(error instanceof Error ? error.message : "作品重命名失败"); } };
  const deleteProject = async () => { if (!currentProjectId || generating || recoverableTask) return; try { const result = await api<{ active_session_id: string; backup: { name: string } }>(`/projects/${encodeURIComponent(currentProjectId)}`, { method: "DELETE" }); await refreshProjects(); await switchSession(result.active_session_id); notify(`作品已删除，安全备份已创建：${result.backup.name}`, "success"); } catch (error) { notify(error instanceof Error ? error.message : "作品删除失败"); } };
  const switchProject = async (projectId: string) => { try { const result = await api<{ session: ChatSession; messages: ChatMessage[] }>("/project/switch", { method: "POST", body: JSON.stringify({ project_id: projectId }) }); setCurrentProjectId(projectId); useChatStore.getState().setActiveSession(result.session.id, result.messages); await refreshSessions(projectId); await loadProjectSettings(projectId); await loadChapters(projectId); await refreshGenerationTasks(result.session.id); useModeStore.getState().setLocalMode(result.session.mode); } catch (error) { notify(error instanceof Error ? error.message : "作品切换失败"); } };
  ...
  const removeChapter = async (chapterId: string) => { const result = await api<{ affected_nodes?: ImpactHighlight[] }>(`/chapter/delete?chapter_id=${encodeURIComponent(chapterId)}`, { method: "DELETE" }); if (result.affected_nodes) setImpactHighlights(result.affected_nodes); if (currentProjectId) await loadChapters(currentProjectId); notify("章节已移入回收站", "success"); };
  const loadChapterHistory = async (chapterId: string) => { try { return await api<ChapterVersion[]>(`/chapter/history?chapter_id=${encodeURIComponent(chapterId)}`); } catch (error) { notify(error instanceof Error ? error.message : "读取章节历史失败"); return []; } };
  const restoreChapterVersion = async (versionId: string) => { try { const result = await api<{ affected_nodes?: ImpactHighlight[] }>(`/chapter/version/${encodeURIComponent(versionId)}/restore`, { method: "POST" }); if (result.affected_nodes) setImpactHighlights(result.affected_nodes); if (currentProjectId) await loadChapters(currentProjectId); notify("章节已恢复", "success"); return true; } catch (error) { notify(error instanceof Error ? error.message : "恢复章节失败"); return false; } };
  const purgeDeletedChapter = async (versionId: string) => { try { await api(`/chapter/trash/${encodeURIComponent(versionId)}`, { method: "DELETE" }); if (currentProjectId) await loadChapters(currentProjectId); notify("回收站记录已永久删除", "success"); return true; } catch (error) { notify(error instanceof Error ? error.message : "永久删除失败"); return false; } };
  const loadChapterDraft = useCallback(async (chapterId: string) => { try { return await api<ChapterDraft | null>(`/chapter/draft?chapter_id=${encodeURIComponent(chapterId)}`); } catch (error) { notify(error instanceof Error ? error.message : "读取自动保存草稿失败"); return null; } }, [notify]);
  const saveChapterDraft = useCallback(async (draft: Pick<ChapterDraft, "chapter_id" | "title" | "content" | "source_updated_at">) => { try { await api<ChapterDraft>("/chapter/draft", { method: "PUT", body: JSON.stringify(draft) }); return { ok: true }; } catch (error) { return { ok: false, message: error instanceof Error ? error.message : "自动保存失败" }; } }, []);
  const clearChapterDraft = useCallback(async (chapterId: string) => { try { await api(`/chapter/draft?chapter_id=${encodeURIComponent(chapterId)}`, { method: "DELETE" }); return true; } catch (error) { notify(error instanceof Error ? error.message : "清理草稿失败"); return false; } }, [notify]);
  const createRule = async (key: string, value: string, category: UniverseRule["category"]) => { if (!currentProjectId) return; await api("/universe/rule", { method: "POST", body: JSON.stringify({ project_id: currentProjectId, key, value, category, immutable: false }) }); await loadProjectSettings(currentProjectId); };
  ...
  const updateSettings = async (settings: ProjectSettingsConfig) => { if (!currentProjectId) return; try { const updated = await api<Project>(`/projects/${encodeURIComponent(currentProjectId)}/settings`, { method: "PATCH", body: JSON.stringify(settings) }); setProjects((current) => current.map((project) => project.id === updated.id ? { ...project, ...updated, active: project.active } : project)); } catch (error) { notify(error instanceof Error ? error.message : "创作配置保存失败"); } };
  const updateProjectStatus = async (status: Project["status"]) => { if (!currentProjectId) return; try { const updated = await api<Project>(`/projects/${encodeURIComponent(currentProjectId)}/status`, { method: "POST", body: JSON.stringify({ status }) }); setProjects((current) => current.map((project) => project.id === updated.id ? { ...project, ...updated, active: project.active } : project)); notify(status === "archived" ? "作品已归档，可随时恢复" : "作品已恢复", "success"); } catch (error) { notify(error instanceof Error ? error.message : "作品状态更新失败"); } };
