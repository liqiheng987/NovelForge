...
import type { BranchComparison, ChatMessage, ChatSession, CreationAction, Mode, Paper, Project, ProjectSettings as Settings } from "../types";
  ...
  return (
    <section className={`paper-window ${paper.status}`} data-message-id={messageId}>
      ...
      <footer>
        ...
        {paper.status === "draft" && (
          <div>
            <button disabled={busy} type="button" onClick={onAbandon}>放弃</button>
            <button disabled={busy} type="button" onClick={onModify}><FilePenLine size={14} /> 修改</button>
            <button className="paper-confirm" disabled={busy} type="button" onClick={onConfirm}>{busy ? <LoaderCircle className="spin" size={14} /> : <Check size={14} />}确认收录</button>
          </div>
        )}
        {paper.status === "collected" && <div><button className="paper-continue" type="button" onClick={onContinue}><Sparkles size={14} />承接本章继续写</button></div>}
      </footer>
    </section>
  );
}
...
export default function CenterPanel({
  ...
  const submit = async () => {
    ...
    if (!value || generating || !activeSessionId) return;
    setInput("");
    ...
    <main className="workspace-panel chat-panel glass">
      <header className="chat-header">
        ...
        <div className="session-controls">
          ...
          <ModeSelector disabled={generating || !activeSessionId} mode={mode} onChange={(value) => void onModeChange(value)} />
        </div>
      </header>
      ...
      <section className={`creator-toolbox ${activeTool ? "expanded" : ""}`}>
        ...
        {activeTool && <div className="creator-toolbox-content">
          ...
          {activeTool === "branches" && <BranchManager disabled={generating} sessions={sessions} currentSessionId={activeSessionId} onCreate={onCreateBranch} onSwitch={onSwitchSession} onCompare={onCompareBranches} onMerge={onMergeBranches} />}
              ...
              <div className="message-body">
                ...
                <div className="message-actions">
                  ...
                  {isLastAssistant && previousUser && message.paper?.status !== "collected" && (
                    <button disabled={generating} type="button" onClick={() => void onRegenerate(previousUser.content, previousUser.selected_material_ids, message.id)}><RefreshCcw size={13} />重新生成</button>
                  )}
                </div>
                {message.paper && (
                  <PaperWindow
      ...
      <div className="chat-composer">
        ...
        <div className="composer-workflow-actions">
          <button className={creationAction === "discuss" ? "active" : ""} disabled={generating} type="button" onClick={() => chooseTemplate("discuss", "帮我梳理这部新小说的核心设定、主角目标、主要冲突、整体阶段和前三章推进方案。只做规划，先不要生成正文。")}>规划新小说</button>
          <button className={creationAction === "create" ? "active" : ""} disabled={generating} type="button" onClick={() => chooseTemplate("create", "根据当前设定、素材和讨论，生成一篇正式开篇。建立主角困境、核心冲突与行动选择，结尾留下可继续发展的钩子。")}>生成正式开篇</button>
          <button className={creationAction === "continue" ? "active" : ""} disabled={generating} type="button" onClick={() => chooseTemplate("continue", "承接上一章结尾，生成下一章正式篇章。保持人物状态、时间顺序和未解线索连续，推进一个核心事件。")}>续写下一章</button>
          <button disabled={generating} type="button" onClick={() => chooseTemplate("continue", "直接承接已收录的最后一章，连续生成后面10章，并将每章通过宇宙铁律、连续性和篇幅检查后直接收入篇章，无需逐章确认。")}>连续生成10章</button>
          {(creationAction === "create" || creationAction === "continue" || paperSourceMessageId) && <label><span>目标</span><select disabled={generating} value={chapterTargetWords} onChange={(event) => setChapterTargetWords(Number(event.target.value))}><option value={1500}>约 1500 字</option><option value={3000}>约 3000 字</option><option value={5000}>约 5000 字</option><option value={8000}>约 8000 字</option></select></label>}
        </div>
