...
export default function InspirationGenerator({ apiConfig, projectId, onSelect }: { apiConfig: Record<string, string>; projectId: string | null; onSelect: (value: string) => void }) {
  ...
  return <section className="inspiration-generator"><header><Lightbulb size={14} /><strong>灵感生成器</strong></header><input value={premise} onChange={(event) => setPremise(event.target.value)} placeholder="当前设定或困境" /><input value={dilemma} onChange={(event) => setDilemma(event.target.value)} placeholder="希望解决的冲突" /><button disabled={busy || !premise.trim()} type="button" onClick={() => void generate()}>{busy ? "生成中…" : "生成 10 个方向"}</button>{options.length > 0 && <ol>{options.map((option, index) => { const text = [option.title, option.hook, option.conflict].filter(Boolean).join("："); return <li key={index}><button type="button" onClick={() => onSelect(text)}><strong>{option.title || `方向 ${index + 1}`}</strong><span>{option.hook || option.conflict}</span></button></li>; })}</ol>}</section>;
}
