...
export default function CrossBridge({ apiConfig }: { apiConfig: Record<string, string> }) {
  ...
  const run = async () => {
    ...
    setError("");
    try {
      setResult(await api("/cross/bridge", { method: "POST", body: JSON.stringify({ source_text: sourceText, source_type: sourceType, target_type: targetType, source_language: "zh", target_language: targetLanguage, api_config: apiConfig }) }));
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "转译失败，请稍后重试");
